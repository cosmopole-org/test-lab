...
impl Executor {
    ...
    pub fn create(config: Option<&Config>, stat: Option<&Statistics>) -> WasmEdgeResult<Self> {
        let conf_ctx = config.map(|cfg| cfg.inner.0).unwrap_or(std::ptr::null_mut());

        let stat_ctx = stat.map(|stat| stat.inner.0).unwrap_or(std::ptr::null_mut());
...
impl Executor {
    ...
    pub fn call_func(
        ...
        let func_ty = func.ty().ok_or(WasmEdgeError::Func(wasmedge_types::error::FuncError::Type))?;
    ...
    pub fn call_func_with_timeout(
        ...
        unsafe {
            ...
            if libc::timer_create(libc::CLOCK_REALTIME, &mut sev, &mut timerid) < 0 {
                return Err(Box::new(error::WasmEdgeError::Operation("timer_create error".into())));
            }
            ...
            if libc::timer_settime(timerid, 0, &value, std::ptr::null_mut()) < 0 {
                ...
                return Err(Box::new(error::WasmEdgeError::Operation("timer_settime error".into())));
            }
    ...
    pub async fn call_func_async(
        ...
        FiberFuture::on_fiber(async_state, || self.call_func(func, params)).await.unwrap()
    }
    ...
    pub async fn call_func_async_with_timeout(
        ...
        TimeoutFiberFuture::on_fiber(
            async_state,
            || self.call_func(func, params),
            ldd
        ).await.map_err(|_| Box::new(error::WasmEdgeError::ExecuteTimeout))?
    }
    ...
    pub async fn call_func_ref_async<FuncRef: AsFunc + Send>(
        ...
        FiberFuture::on_fiber(async_state, || self.call_func_ref(func_ref, params)).await.unwrap()
    }
}
...
impl Executor {
    ...
    pub fn register_import_module<T: AsInstance + ?Sized>(
        ...
        unsafe {
            check(
                ffi::WasmEdge_ExecutorRegisterImport(self.inner.0, store.inner.0, import.as_ptr())
            )?;
        }
